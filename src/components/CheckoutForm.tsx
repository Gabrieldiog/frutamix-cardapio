...
export default function CheckoutForm() {
    ...
    const handleGeoThenSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (!validate()) return;
        const geoGranted = localStorage.getItem('frutamix-geo-granted');
        if (!geoGranted) {
    ...
    const handleGeoAllow = () => {
        localStorage.setItem('frutamix-geo-granted', 'true');
    ...
    const doSubmit = async () => {
        ...
        try {
            ...
            // Salvar localização do cliente no Firebase para rastreamento
            try {
                navigator.geolocation.getCurrentPosition(
                    (pos) => {
                        const clientRef = ref(db, `client_locations/${data.order.id}`);
                        set(clientRef, {
    ...
    return (
        <>
            ...
            <form onSubmit={handleGeoThenSubmit}>
                {/* Order Summary */}
